...
class Process
{
   ...
   public function set_mas($mas)
    {
        $this->mas = $mas; 
        //error_log(var_dump($mas)); 
        }  

    public static function makeProcess($mysqli,$input,$feed,$redis)
    {
    ...
    public function get_process_list()
    {
        ...
        $list[40] = array(_("Reverse Sync"),ProcessArg::INPUTID,"reverse_sync",0,DataType::UNDEFINED,"Misc");
        $list[41] = array(_("Energy"),ProcessArg::FEEDID,"verify_energy",1,DataType::REALTIME,"Power",array(Engine::PHPFIWA,Engine::PHPFINA,Engine::PHPTIMESERIES));                  
        //Water
        $list[42] = array(_("L/s to Liters"),ProcessArg::FEEDID,"ls_to_liters",1,DataType::REALTIME,"Water",array(Engine::PHPFINA,Engine::PHPTIMESERIES));  
        $list[43] = array(_("L/s to L/d"),ProcessArg::FEEDID,"ls_to_lsd",1,DataType::DAILY,"Water",array(Engine::PHPTIMESERIES));
	$list[44] = array(_("L to L/d"),ProcessArg::FEEDID,"kwh_to_kwhd",2,DataType::DAILY,"Water",array(Engine::PHPTIMESERIES));             
	return $list;
    }
   ...
   public function divide_by_feed($feedid, $time, $value)
    ...
    public function cossmic_sync($arg, $time, $value)
    {
        ...
        public function reverse_sync($arg, $time, $value)
    {
        global $cossmic;
        ...
        if ($input) {
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $cossmic['host'].'feed/getid.json?name='.$cossmic['node'].'_neighborhood_out_kwh&apikey='.$cossmic['apikey']);
           curl_setopt($ch, CURLOPT_HEADER, 0);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);
            curl_setopt($ch, CURLOPT_TIMEOUT, 5);
            ...
            if(is_numeric($id))
            {
                ...
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $cossmic['host'].'feed/value.json?id='.$id.'&apikey='.$cossmic['apikey']);
                curl_setopt($ch, CURLOPT_HEADER, 0);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);
                curl_setopt($ch, CURLOPT_TIMEOUT, 5);
    ...
    {
         $this->mas->updateLearning($id,$time,$value);
        return $value;     
    ...
    public function verify_energy($feedid, $time, $value)
    {
        global $redis;
        if (!$redis) return $value; // return if redis is not available
        ...
        else $redis->hMset("process:kwhtopower:$feedid", array('time' => $time, 'value' => $value));
    ...
    public function household($arg, $start, $value)
    {
        ...
        $household_inputs = $this->verify_household_inputs($userid);
        ...
        if ($grid_out['time'] > $last['time'] and 
            ...
            if ($pv_diff > 0) {
                ...
                if ($pv2household < 0) {
                    $this->log->info("Household: Offset for pv2household=$pv2household");
                }
                if ($pv_diff != $pv2grid + $pv2household + $pv2storage) {
                    $this->log->info("Household: Mismatch calculation for PV unequal pv2grid, pv2household, pv2storage");
                }
            }
            else {
                $time = $grid_out['time'];
    ...
    private function verify_household_inputs($userid) {

        $inputs = $this->input->get_inputs($userid)[0];

        $household_inputs = Array('grid_out', 'grid_in', 'storage_out', 'storage_in', 
                'grid2household', 'grid2storage', 'storage2grid', 'storage2household',
                'pv2grid', 'pv2household', 'pv2storage', 'pv', 'consumption');
        
        $newinput = false;
        foreach ($household_inputs as $name) {
            if (!isset($inputs[$name])) {
                $inputid = $this->input->create_input($userid, 0, $name);
                $this->verify_household_feeds($userid, $inputid, $name, 'energy');
            }
        }
    ...
    private function verify_household_feeds($userid, $inputid, $name, $type) {

        $this->log->info("Set process list for input ".$name."...\r\n");
        ...
        if ($type === 'energy') {
            $this->input->add_process($this, $userid, $inputid, 41, $kwhid);
            $this->input->add_process($this, $userid, $inputid, 21, $powerid);
            $this->input->add_process($this, $userid, $inputid, 5, $kwhdid);
        }
        else if ($type === 'power') {
            $this->input->add_process($this, $userid, $inputid, 1, $powerid);
            $this->input->add_process($this, $userid, $inputid, 4, $kwhid);
            $this->input->add_process($this, $userid, $inputid, 23, $kwhdid);
        }
    }

    private function write_household_input($input, $time, $value) {
        $last_value = $this->get_household_value($input, 'energy');
        if ($value > 0) {
            $new_value = $last_value['value'] + $value;
        }
        else {
            $new_value = $last_value['value'];
        }
        $this->write_to_input($input['id'], $time, $new_value);
    }
    
    private function get_household_value($input, $type) {
        if ($type === 'power') {
            $processid = '21';
        }
        else $processid = '1';
        ...
        foreach ($processes as &$process) {
            $par = explode(':', $process);
            // Look for log process id
            if ($par[0] == $processid) {
                return $this->feed->get_timevalue($par[1]);
            }
        }
    }

public function ls_to_liters($feedid, $time_now, $value)
    {
        $new_kwh = 0;
        ...
        if ($last_time && (time()-$last_time)<7200)
        {
            // kWh calculation
            $time_elapsed = ($time_now - $last_time);
            $kwh_inc = ($time_elapsed * $value) / 1000.0;
            $new_kwh = $last_kwh + $kwh_inc;
        } else {
            // in the event that redis is flushed the last time will
            // likely be > 7200s ago and so kwh inc is not calculated
 ...
 public function ls_to_lsd($feedid, $time_now, $value)
    {
        $new_kwh = 0;
