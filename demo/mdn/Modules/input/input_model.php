...
class Input
{
    ...
    public function get_timevalue($id)
    {
        ...
        if ($this->redis) {
            return $this->redis->hMget("input:lastvalue:$id", array('value', 'time'));
        } else {
    ...
    {
		error_log(" ".$userid." ".$inputid." ".$processid." ".$arg);
        $userid = (int) $userid;
        $inputid = (int) $inputid;
    ...
    ******/
    ...
    public function reset_process($id)
    {
		...
		return array('success'=>true, 'message'=>'Processlist reset');
    }
...
//------------------------------------------
// CoSSMic methods to get info in p-learn
//------------------------------------------

 public function getInput($id)
    {
        if ($this->redis) {
            return $this->redis_getInput($id);
        } else {
            return $this->mysql_getInput($id);
        }
    }

 // USES: redis input
public function redis_getInput($id)
    {
        $row = $this->redis->hGetAll("input:$id");
        $lastvalue = $this->redis->hmget("input:lastvalue:$id",array('time','value'));
        $row['time'] = $lastvalue['time'];
 ...
 // USES: redis input
public function mysql_getInput($id)
    {
      
       $result = $this->mysqli->query("SELECT id,nodeid,name,description,processList,time,value,userid FROM input WHERE `id` = '$id'");
